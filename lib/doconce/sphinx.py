...
def _usage_sphinx_dir():
    print("Usage:\n"
          "doconce sphinx_dir [<arguments>] [intersphinx] <file.do.txt> [--runestone]\n"
          "\n"
          ...
          "favicon=<icon-file>\n"
          "\n"
          "The sphinx_dir command creates a directory <dir> for compiling a sphinx version of a doconce document.\n"
          '\n'
          "The default argument <dir> to dirname is \"sphinx-rootdir\". The option --runestone makes a directory \n"
          "for RunestoneInteractive documents (slightly different from a standard sphinx directory).\n"
          "\n"
          "The intersphinx option is used for links to other sphinx documents.\n"
          "\n"
          "toc_depth governs the depth in the table of contents, default value is 2 (sections and subsections are \n"
          "included).\n"
          "\n"
          ...
          "\n"
          "The theme option can be chosen from basic sphinx themes (agogo, basic, bizstyle, classic, default, epub, \n"
          "haiku, nature, pyramid, scrolls, sphinxdoc, traditional), extra themes that can be installed (alabaster, \n"
          "bootstrap, cloud, redcloud, solarized, sphinx_rtd_theme, draft, flat, modern5, minimal5, responsive), \n"
          "or themes that come with DocOnce (ADCtheme, agni, basicstrap, bloodish, cbc, fenics, fenics_classic, \n"
          "fenics_minimal1, fenics_minimal2, jal, pylons, scipy_lectures, slim-agogo, uio, uio2, vlinux-theme). \n"
          "See bundled/sphinx_themes/install.txt for how to install extra themes.\n"
          "\n"
          "A custom theme can also be specified by giving a \"_themes-like\" directory: theme_dir=<dirname>, where \n"
          "<dirname> can be any path (DocOnce will copy this directory to the sphinx root directory). Note that this \n"
          "must be the *theme directory* with files like layout.html and theme.conf (not the entire Python package \n"
          "for a theme). Alternatively, one can use a standard theme, but provide one's own tailored conf.py file \n"
          "through the conf.py=filename option.\n" 
          "If the file (filename) has empty strings for the project and copyright, those values as well as the\n"
          "variables depending on these values will get content based on the title, author, and copyright from the\n"
          "DocOnce source file mydoc.do.txt. (It is recommended to leave project and copyright as empty strings, \n"
          "and latex_documents, texinfo_documents, and man_pages as empty lists.)\n"
          "\n"
          "---\n"
          "\n"
          "The steps for copying files to the sphinx directory and compiling the document is done by an\n"

...
admon_styles1 = admon_styles_text + \
                ('.notice, .summary, .warning, .question, .block {\n'
                 ...
                 'background-image: url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/%(icon_notice)s); }\n'
                 '.summary  { color: #4F8A10; background-color: %(background_summary)s;\n'
                 'background-image:url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/%(icon_summary)s); }\n'
                 '.warning  { color: #9F6000; background-color: %(background_warning)s;\n'
                 'background-image: url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/%(icon_warning)s); }\n'
                 '.question { color: #4F8A10; background-color: %(background_question)s;\n'
                 'background-image:url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/%(icon_question)s); }\n'
...
admon_styles2 = admon_styles_text + (
    ...
    '.alert-notice { background-image: url(RAW_GITHUB_URL/doconce/doconce/bundled/html_images/'
    '%(icon_notice)s); }\n'
    '.alert-summary  { background-image:url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/'
    '%(icon_summary)s); }\n'
    '.alert-warning { background-image: url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/'
    '%(icon_warning)s); }\n'
    '.alert-question {background-image:url(RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/'
    '%(icon_question)s); }\n')
...
def css_link_solarized_highlight(style='light'):
    return ('\n'
            '<link href="RAW_GITHUB_URL/doconce/doconce/main/bundled/html_styles/style_solarized_box/css/'
            'solarized_%(style)s_code.css" rel="stylesheet" type="text/css" title="%(style)s"/>\n'
            '<script src="RAW_GITHUB_URL/doconce/doconce/main/bundled/html_styles/style_solarized_box/'
    ...
    if icon.lower() != 'none':
        if icon == 'default':
            if html_style == 'solarized' or html_style == 'bloodish':
                ...
                icon = 'exercise1.svg'
        icon_path = 'RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/' + icon
...
def html_quiz(quiz):
    ...
    for i, choice in enumerate(quiz['choices']):
        ...
        if not bootstrap:  # plain html: show tooltip when hovering over choices
            ...
            collapsed_text = '<img src="RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/%s.gif">\n%s' % ('correct' if choice[0] == 'right' else 'incorrect', expl)
...
for _admon in globals.admons:
    ...
    _text = ('\n'
             ...
             '<img src="RAW_GITHUB_URL/doconce/doconce/main/bundled/html_images/lyx_%(_admon)s.png" hspace="5" '
             'alt="%(_admon)s"></td>\n'
...
def define(FILENAME_EXTENSION,
    ...
    if html_style.startswith('boots'):
        ...
        if html_style == 'bootstrap':
            ...
            urls = ['https://netdna.bootstrapcdn.com/bootstrap/%s/css/bootstrap.min.css' % boots_version,
                    'RAW_GITHUB_URL/doconce/doconce/main/bundled/html_styles/style_bootstrap/css/%s.css' % html_style]
    ...
    if option('pygments_html_style=', 'default') == 'highlight.js':
        scripts += ('\n'
                    ...
                    '<script src="RAW_GITHUB_URL/doconce/doconce/main/bundled/html_styles'
